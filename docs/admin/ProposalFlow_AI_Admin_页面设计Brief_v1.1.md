...
12. **平台边界冻结**：admin 端同样只做 desktop / laptop web app，不做 mobile 或 tablet 产品体验；设计稿不应扩写移动端或平板端专用布局、交互或验收标准。
13. **User Detail 首版策略冻结**：只作为 P1 预留，不进入首版正式交付。
14. **Funnel → 列表跳转冻结**：点击漏斗节点跳转到过滤后的 Workspaces 列表属于增强项，不进入首版 P0。
15. **Subscriptions 时间筛选冻结**：首版统一沿用列表协议中的 `from / to`，不新增专门针对 `current_period_end` 的特殊筛选字段命名。
16. **Workspace Detail 活动时间线冻结**：首版不单独拆活动接口，时间线折叠进 workspace detail 查询结果。
